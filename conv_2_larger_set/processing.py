    ...
    for i in range(num_chrom):
        ...
        for j in range(piece_size):
            ...
            if index_1 >= 0 and index_1 < models.len_chrom:
                piece_1[j] = int(out[i][index_1])
            
            index_2 = (j - piece_size//2) + ind2
            if index_2 >= 0 and index_2 < models.len_chrom:
                piece_2[j] = int(out[i][index_2])
            
            index_3 = (j - piece_size//2) + ind3
            if index_3 >= 0 and index_3 < models.len_chrom:
                piece_3[j] = int(out[i][index_3])

        for j in range(3):
            local_1[j].append([0]*piece_size)
            local_2[j].append([0]*piece_size)
            local_3[j].append([0]*piece_size)
        
        for j in range(piece_size):
            
            local_1[piece_1[j]][i][j] = 1
            local_2[piece_2[j]][i][j] = 1
            local_3[piece_3[j]][i][j] = 1
    
    
    if (random.random() < 0.5):
        local_1 = [local_1[2], local_1[1], local_1[0]]
        local_2 = [local_2[2], local_2[1], local_2[0]]
        local_3 = [local_3[2], local_3[1], local_3[0]]
...
def create_input1(sample_file, command_file):
    ...
    local_1_n = [local_1[2], local_1[1], local_1[0]]
    local_2_n = [local_2[2], local_2[1], local_2[0]]
    local_3_n = [local_3[2], local_3[1], local_3[0]]


    return [local_1 + local_2, local_2 + local_1, local_1_n + local_2_n, local_2_n + local_1_n,
    local_2 + local_3, local_3 + local_2, local_2_n + local_3_n, local_3_n + local_2_n,
    local_3 + local_1, local_1 + local_3, local_3_n + local_1_n, local_1_n + local_3_n]
...
def convert_command_file1(file):
    ...
    return [flist,flist,flist, flist,flist,flist, flist,flist,flist, flist,flist,flist]