...
if saving:
    X = [create_input1("../../data/sampled_genotypes/sample_" + str(i), "../../data/commands/command_" + str(i)) for i in range(num_files)]
    X = [x for x in X if x is not None]
    X = torch.tensor(X).float()
    torch.save(X, "X100k")

    C = [convert_command_file1("../../data/commands/command_" + str(i)) for i in range(num_files)]
...
X = X.reshape(num_files*12, 6, num_chrom, piece_size)
C = C.reshape(num_files*12, 11)
