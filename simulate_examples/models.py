...
class TransformerModel1(nn.Module):
   ...
   def forward(self, x):
       ...
       x = x.reshape(x.shape[0], num_chrom, len_chrom*n_embd) #(batch, num_chrom, len_chrom * n_embd)
       x = self.ln1(x) #(batch, num_chrom, len_chrom * n_embd)
       x = self.linear(x) #(batch, num_chrom, len_chrom * n_embd)


       x = x.reshape(x.shape[0], num_chrom*len_chrom*n_embd) #(batch, -1)
       x = self.ln2(x)


       y1 = self.linear2_1(x) #(batch, 100)
       y1 = self.relu(y1)
       y1 = self.linear3_1(y1) #(batch, 1)
       y1 = y1.reshape(-1) #(batch)
...
class SimpleModel(nn.Module):
   ...
   def __init__(self):
       super().__init__()
       self.linear1 = nn.Linear(num_chrom*len_chrom*n_embd, 250)


       self.ln = nn.LayerNorm(250)


       self.linear2_1 = nn.Linear(250,1)
       self.linear2_2 = nn.Linear(250,1)
       self.relu = nn.ReLU()
