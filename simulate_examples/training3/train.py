...
X1 = [convert_sampling_file("../../test_training/sampled_genotypes/sample_stronger_" + str(i)) for i in range(num_files)]
X1 = torch.tensor(X1) - 1
GetMemory()
X1 = X1.repeat_interleave(2,dim=0)
print(X1.shape)

X2 = [convert_command_file5("../../test_training/commands/command_stronger_" + str(i)) for i in range(num_files)]
X2 = torch.tensor(X2)
