...
X = [convert_file("../splits/split_" + str(i), "../commands/command_" + str(i)) for i in range(num_files)]


X = torch.tensor(X) #
print(X.shape)
GetMemory()
X = X.reshape(-1,X.shape[2],X.shape[3]) #Combine the first two dimensions (this reshpaes exactly as expected)
X = X.float()
